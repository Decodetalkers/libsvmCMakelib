...
public:
  ...
  void Solve(int l, const QMatrix &Q, const double *p, const schar *y,
             double *alpha, double Cp, double Cn, double eps, SolutionInfo *si,
             int shrinking) {
    this->si = si;
...
static void solve_nu_svr(const svm_problem *prob, const svm_parameter *param,
  ...
  Solver_NU s;
  s.Solve(2 * l, SVR_Q(*prob, *param), linear_term, y, alpha2, C, C, param->eps,
          si, param->shrinking);
...
// indices to the original data perm, length l, must be allocated before calling
// this subroutine
static void svm_group_classes(const svm_problem *prob, int *nr_class_ret,
  ...
  // we swap labels to ensure that internally the binary SVM has positive data
  // corresponding to the +1 instances.
  //
...
svm_model *svm_train(const svm_problem *prob, const svm_parameter *param) {
  ...
  if (param->svm_type == ONE_CLASS || param->svm_type == EPSILON_SVR ||
    ...
    if (nr_class == 1)
      info("WARNING: training data in only one class. See README for "
           "details.\n");
...
void svm_cross_validation(const svm_problem *prob, const svm_parameter *param,
  ...
  if (nr_fold > l) {
    nr_fold = l;
    fprintf(stderr, "WARNING: # folds > # data. Will use # folds = # data "
                    "instead (i.e., leave-one-out cross validation)\n");
  }
...
bool read_model_header(FILE *fp, svm_model *model) {
  ...
  // parameters for training only won't be assigned, but arrays are assigned as
  // NULL for safety
  param.nr_weight = 0;
